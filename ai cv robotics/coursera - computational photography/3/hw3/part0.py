...
def reduce(image):
  ...
  r = image.shape[0]
  c = image.shape[1]
  out = np.zeros((math.ceil(r/2), math.ceil(c/2)), dtype = float)
  kernel = generating_kernel(0.4)
  filtered_image = convolve2d(image, kernel)
  filtered_image = filtered_image[2:filtered_image.shape[0]-2,2:filtered_image.shape[1]-2]
  filtered_image1 = filtered_image[np.arange(0,r, 2), :]
  out = filtered_image1[:, np.arange(0,c, 2)]
...
def expand(image):
  ...
  r = image.shape[0]
  c = image.shape[1]
  out = np.zeros((2*r,2*c), dtype = float)
  kernel = generating_kernel(0.4)
  upsampled = np.zeros((2*r,2*c), dtype = float)
  upsampled[::2,::2] = image
  filtered_image = convolve2d(upsampled, kernel)
  out = 4*filtered_image[2:filtered_image.shape[0]-2,2:filtered_image.shape[1]-2]
  # Insert your code here ------------------------------------------------------
