...
def interlace(evens, odds):
  ...
  outimg = None
  outimg = (np.random.rand(evens.shape[0]+odds.shape[0],evens.shape[1],3) * 255).astype(np.uint8)
  outimg[np.arange(0,outimg.shape[0], 2), :, :] = evens
  outimg[np.arange(1,outimg.shape[0], 2), :, :] = odds
