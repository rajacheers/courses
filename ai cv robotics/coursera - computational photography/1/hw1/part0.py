...
def convolve(image, kernel):
  ...
  output_dummy = np.zeros((rows - d + 1, cols - d + 1), dtype = float)
  kernel_reversed = kernel[::-1,::-1].astype(float)
  for i in range(0,output.shape[0]):
    for j in range(0,output.shape[1]):
       image_patch = image[i:i+d,j:j+d].astype(float)
       output_dummy[i,j] = (image_patch * kernel_reversed).sum()
  output = output_dummy.clip(min = 0, max = 255).astype(np.uint8)
