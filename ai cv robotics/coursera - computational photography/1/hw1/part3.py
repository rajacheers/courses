...
def filter_median(image, k):
  ...
  rows = image.shape[0]
  cols = image.shape[1]
  output = np.zeros((rows - 2*k, cols - 2*k,), dtype = float)
  for i in range(0,output.shape[0]):
    for j in range(0,output.shape[1]):
       image_patch = image[i:i+2*k+1,j:j+2*k+1].astype(float)
       output[i,j] = np.median(image_patch[:])
  output = output.clip(min = 0, max = 255).astype(np.uint8)
